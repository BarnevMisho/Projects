		...
		uint32_t size)
{
	...
	for (position = 0; position < size; position += VL53L8CX_COMMS_CHUNK_SIZE)
	{
		...
		HAL_GPIO_WritePin(p_platform->ncs_gpio_port, p_platform->ncs_pin, GPIO_PIN_RESET);
		status |= HAL_SPI_Transmit(p_platform->hspi, data_write, data_size, 100*data_size);
		HAL_GPIO_WritePin(p_platform->ncs_gpio_port, p_platform->ncs_pin, GPIO_PIN_SET);
	}
		...
		uint32_t size)
{
	...
	for (position = 0; position < size; position += VL53L8CX_COMMS_CHUNK_SIZE)
	{
		...
		HAL_GPIO_WritePin(p_platform->ncs_gpio_port, p_platform->ncs_pin, GPIO_PIN_RESET);
		status |= HAL_SPI_Transmit(p_platform->hspi, data_write, 2, 0x1000);
		status |= HAL_SPI_Receive(p_platform->hspi, p_values + position, data_size, 100*data_size);
		HAL_GPIO_WritePin(p_platform->ncs_gpio_port, p_platform->ncs_pin, GPIO_PIN_SET);
	}
